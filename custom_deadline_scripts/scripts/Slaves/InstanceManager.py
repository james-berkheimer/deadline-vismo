...
def __main__():
    ...
    slavePools = [str(thing) for thing in slave_settings.SlavePools]
...
def GetSlaveIni():
    global selectedSlave
    slaveInidir = {}
    pathToSlaveDir = "\c$\ProgramData\Thinkbox\Deadline10\slaves\\"
...
def SpawnInstances():
    ...
    for i in range(scriptDialog.GetValue( "InstancesBox" )):
        ...
        RepositoryUtils.SetPoolsForSlave(str(selectedSlave[0] + '-' + string.ascii_uppercase[i]), slavePools)
