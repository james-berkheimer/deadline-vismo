...
//////////////////////////////////////////
		for( i = 1; i <= app.project.renderQueue.numItems; ++i )
		{
				frameSize = findFrameSize(app.project.renderQueue.item( i ));			
		}		
		// Project Phase
		dialog.phaseGroup = dialog.descPanel.add( 'group', undefined );
		dialog.phaseGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.phaseLabel = dialog.phaseGroup.add( 'statictext', undefined, 'Project Phase' );
		dialog.phaseLabel.size = labelSize;
		dialog.phaseLabel.helpTip = 'DLVP, ALPHA, BETA, FINAL?';
		dialog.phase = dialog.phaseGroup.add( 'edittext', undefined, findPhase(projectPath));
		dialog.phase.size = textSize;
		
////////////////////////////////////////////
		
//////////////////////////////////////////
		// Project Rez
		dialog.rezGroup = dialog.descPanel.add( 'group', undefined );
		dialog.rezGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.rezLabel = dialog.rezGroup.add( 'statictext', undefined, 'Frame Size' );
		dialog.rezLabel.size = labelSize;
		dialog.rezLabel.helpTip = 'Resolution of render';
		dialog.rez = dialog.rezGroup.add( 'edittext', undefined, frameSize);
		dialog.rez.size = textSize;
////////////////////////////////////////////

//////////////////////////////////////////

		// Project Output Type
		dialog.fileTypeGroup = dialog.descPanel.add( 'group', undefined );
		dialog.fileTypeGroup.alignment = [ScriptUI.Alignment.FILL, ScriptUI.Alignment.TOP];
		dialog.fileTypeLabel = dialog.fileTypeGroup.add( 'statictext', undefined, 'Output Type' );
		dialog.fileTypeLabel.size = labelSize;
		dialog.fileTypeLabel.helpTip = 'Resolution of render';
		dialog.fileType = dialog.fileTypeGroup.add( 'edittext', undefined, findFileType());
		dialog.fileType.size = textSize;
...
//////////////////////////////////////////
		...
		dialog.multiMachineTasksLabel = dialog.multiMachineTasksGroup.add( 'statictext', undefined, 'Number Of Tasks' );
		dialog.multiMachineTasksLabel.size = labelSize;
		dialog.multiMachineTasksLabel.enabled = initMultiMachine;
		dialog.multiMachineTasksLabel.helpTip = 'The number of tasks actually represents the number of slaves that can work on this job at the same time. Each slave gets a task, which represents the full frame range, and they will work together until all frames are complete.';
...
//#####################################################################		
		if( dialog.useCompName.value == true)
			...
			removeFileExtension = dialog.jobName.text.slice(0,-4);	
			//jobName = dialog.jobName.text + " - " + jobName;
			//jobName = removeFileExtension + " - " + jobName;
			jobName = removeFileExtension;
//#####################################################################
...
//##########################################################################
		...
		submitInfoFile.writeln( "ExtraInfo7=" + dialog.phase.text );
		submitInfoFile.writeln( "ExtraInfo8=" + dialog.rez.text );
		submitInfoFile.writeln( "ExtraInfo9=" + dialog.fileType.text );
...
//##########################################################################
	...
	function findPhase (projectPath)
	{
		var phase;
		
		if (projectPath.indexOf("ALPHA") >= 0) {
				phase = "ALPHA";
		}
		
		if (projectPath.indexOf("BETA") >= 0) {
				phase = "BETA";
		}
		
		if (projectPath.indexOf("DVLP") >= 0) {
				phase = "DVLP";
		}
		
		if (projectPath.indexOf("FINAL") >= 0) {
				phase = "FINAL";
		}
	...
	function findFileType()
	{
		...
		{		
				for( j = 1; j <= app.project.renderQueue.item( i ).numOutputModules; ++j )
				{
				  ...
				  var outputPrefix = outputFile.substring( 0, index );
				  var outputExt = outputFile.substring( index + 1, outputFile.length );
				  var outputExtUPPER = outputExt.toUpperCase();
				}
	  }
	  return outputExtUPPER;		
	}
