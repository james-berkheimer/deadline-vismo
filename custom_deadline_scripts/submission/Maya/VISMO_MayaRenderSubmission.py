...
###################################################
    ...
    scriptDialog.AddControlToGrid("ProjectLabel","LabelControl","Project Path", 5, 0, "The Maya project folder (this should be a shared folder on the network).", False)
    projBox = scriptDialog.AddSelectionControlToGrid("ProjectBox","FolderBrowserControl","","", 5, 1, colSpan=3)
    projBox.ValueModified.connect(SetDefaultLocation)
    defaultLocation = scriptDialog.GetValue( "ProjectBox" );

    scriptDialog.AddControlToGrid( "SceneLabel", "LabelControl", "Maya File", 6, 0, "The scene file to be rendered.", False )
    sceneBox = scriptDialog.AddSelectionControlToGrid( "SceneBox", "MultiFileBrowserControl", "", "Maya Files (*.ma *.mb)",6, 1, colSpan=3, browserLocation=defaultLocation )

    scriptDialog.AddControlToGrid("OutputLabel","LabelControl","Output Path",7, 0, "The folder where your output will be dumped (this should be a shared folder on the network).", False)
    outputBox = scriptDialog.AddSelectionControlToGrid("OutputBox","FolderBrowserControl", "","", 7, 1, colSpan=3, browserLocation=defaultLocation)
    
    ###################################################
    scriptDialog.AddControlToGrid( "BuildLabel", "LabelControl", "Build To Force", 8, 0, "You can force 32 or 64 bit rendering with this option.", False )
    scriptDialog.AddComboControlToGrid( "BuildBox", "ComboControl", "64bit", ("None","32bit","64bit"), 8, 1 )
    scriptDialog.AddSelectionControlToGrid( "SubmitSceneBox", "CheckBoxControl", False, "Submit Maya Scene File", 8, 2, "If this option is enabled, the scene file will be submitted with the job, and then copied locally to the slave machine during rendering.", colSpan=2 )
    mayaBatchBox = scriptDialog.AddSelectionControlToGrid( "MayaBatchBox", "CheckBoxControl", True, "Use MayaBatch Plugin", 9, 1, "This uses the MayaBatch plugin. It keeps the scene loaded in memory between frames, which reduces the overhead of rendering the job.", False )
    mayaBatchBox.ValueModified.connect(MayaBatchChanged)
    scriptDialog.AddSelectionControlToGrid( "Ignore211Box", "CheckBoxControl", False, "Ignore Error Code 211", 9, 2, "This allows a MayaCmd task to finish successfully even if Maya returns the non-zero error code 211. Sometimes Maya will return this error code even after successfully saving the rendered images.", False )
    scriptDialog.AddSelectionControlToGrid( "LocalRenderingBox", "CheckBoxControl", False, "Enable Local Rendering", 10, 1, "If enabled, the frames will be rendered locally, and then copied to their final network location." )
    scriptDialog.AddSelectionControlToGrid( "StrictErrorCheckingBox", "CheckBoxControl", True, "Strict Error Checking", 10, 2, "Enable this option to have Deadline fail Maya jobs when Maya prints out any 'error' or 'warning' messages. If disabled, Deadline will only fail on messages that it knows are fatal." )
    ###################################################
    
    scriptDialog.AddControlToGrid( "RendererLabel", "LabelControl", "Renderer", 11, 0, "The Maya renderer to use. If you select 'File', the renderer defined in the scene file will be used, but renderer specific options will not be set by Deadline.", False )
    rendererBox = scriptDialog.AddComboControlToGrid( "RendererBox", "ComboControl", "Arnold", ("File","3delight","Arnold","FinalRender","Gelato","Maxwell",
                                                                                              ...
                                                                                              "RendermanRIS","Turtle","Vray","Iray"), 11, 1 )
    rendererBox.ValueModified.connect(RendererChanged)
    
    ###################################################
    scriptDialog.AddControlToGrid( "ArnoldVerboseLabel", "LabelControl", "Arnold Verbosity", 12, 0, "Set the verbosity level for Arnold renders.", False )
    scriptDialog.AddComboControlToGrid( "ArnoldVerboseBox", "ComboControl", "1", ("0","1","2"), 12, 1 )
    scriptDialog.AddSelectionControlToGrid( "CloseOnSubmissionBox", "CheckBoxControl", True, "Close on Submission", 12, 2, "If checked, the submitter will close after job submission is completed." )
...
#######################################################################################
            if scriptDialog.GetValue( "RenderLayerBox" ):
                writer.WriteLine( "Name=%s" % jobName + " - " + scriptDialog.GetValue( "RenderLayerBox" )  )
            else:
