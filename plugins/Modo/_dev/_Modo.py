...
def process_file(deadlinePlugin, file):
    ...
    if not os.path.isabs(file):
        ...
        if ':' not in file:
            ...
            newPath = find_existing_path( deadlinePlugin, file, possible_path, False )
        else:
...
def find_existing_path( deadlinePlugin, file, sceneDir, isOutputDir ):
    compareVal = remove_padding( file )
    ...
    newPath = RepositoryUtils.CheckPathMapping(projectDir + '/' + compareVal)
    if os.path.isfile(newPath) or ( isOutputDir and not projectDir == ""):
        ...
        results = RepositoryUtils.CheckPathMapping(projectDir + '/' + file)
    else:
        newPath = RepositoryUtils.CheckPathMapping(sceneDir + '/' + compareVal)
        if os.path.isfile( newPath ) or  isOutputDir:
            deadlinePlugin.LogWarning('Replace relative path with new path '+ newPath)
            results = RepositoryUtils.CheckPathMapping(sceneDir + '/' + file)
        else:
            newPath = file
            deadlinePlugin.LogWarning('Unable to find file  ' + RepositoryUtils.CheckPathMapping(sceneDir + '/' + file) )
                ...
                if name.lower() == 'filename' or name == 'file' or name == 'pattern' or name == 'irrLName' or name == 'irrSName' or name == "filePath" or name == "filePattern" or name == "scene":
                    ...
                    if not os.path.isabs(value):
                        ...
                        if ':' not in value:
                            ...
                            value = find_existing_path( deadlinePlugin, value, possible_path, (itemtype == "renderOutput" ) )
                        else:
