...
def __main__( *args ):
    ...
    scriptDialog.AddControlToGrid("ProjectLabel","LabelControl","Project Directory", 6, 0, "The Maya project folder (this should be a shared folder on the network).", False)
    ...
    scriptDialog.AddControlToGrid("OutputLabel","LabelControl","Output Folder",8, 0, "The folder where your output will be dumped (this should be a shared folder on the network).", False)
    ...
    scriptDialog.AddControlToGrid( "FramesLabel", "LabelControl", "Frame List", 12, 0, "The list of frames to render.", False )
    ...
    scriptDialog.AddControlToGrid( "RendererLabel", "LabelControl", "Renderer", 13, 2, "The Maya renderer to use. If you select 'File', the renderer defined in the scene file will be used, but renderer specific options will not be set by Deadline.", False )
    ...
    scriptDialog.AddControlToGrid( "VersionLabel", "LabelControl", "Version", 14, 0, "The version of Maya to render with.", False )
    versionBox = scriptDialog.AddComboControlToGrid( "VersionBox", "ComboControl", supported_versions[-1], supported_versions, 14, 1 )
    versionBox.ValueModified.connect( VersionChanged )
    scriptDialog.AddControlToGrid( "BuildLabel", "LabelControl", "Build To Force", 14, 2, "You can force 32 or 64 bit rendering with this option.", False )
    scriptDialog.AddComboControlToGrid( "BuildBox", "ComboControl", "None", ( "None", "32bit", "64bit" ), 14, 3 )

    ###################################################
    scriptDialog.AddControlToGrid(
        "ArnoldVerboseLabel", "LabelControl", "Arnold Verbosity", 15, 0,
...
def SubmitJob(renderlayer, *args):
    ########################################################################
    logging.debug('I am in SubmitJob \n')
    ########################################################################
    global scriptDialog
    global integration_dialog
