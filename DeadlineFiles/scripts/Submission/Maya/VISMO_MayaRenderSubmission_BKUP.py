...
def __main__(*args):
    ...
    scriptDialog.AddRangeControlToGrid("ChunkSizeBox", "RangeControl", 1, 1,
                                       1000000, 0, 1, 2, 1)
    scriptDialog.AddControlToGrid(
        "CameraLabel", "LabelControl", "Camera (optional)", 3, 0,
        "Leave blank to render using the default camera settings.", False)
    scriptDialog.AddComboControlToGrid("CameraBox",
                                       "ComboControl",
                                       "", ("none", ),
                                       3,
                                       1,
                                       colSpan=1)
    scriptDialog.AddControlToGrid(
        "RenderLayerLabel", "LabelControl", "Render Layer (optional)", 4, 0,
        "Leave blank to render using the default Render Layer settings.",
        False)
    scriptDialog.AddComboControlToGrid("RenderLayerBox",
                                       "ComboControl",
                                       "", ("none", ),
                                       4,
                                       1,
                                       colSpan=1)
    mayaRenderLayersBox = scriptDialog.AddSelectionControlToGrid(
        "SubmitAllRenderLayersBox", "CheckBoxControl", False,
        "Submit All Render Layers", 5, 1,
        "If enabled, all render layers in the list will be submited as deadline jobs.",
        False)
    mayaRenderLayersBox.ValueModified.connect(MayaRenderLayersChanged)
    scriptDialog.AddSelectionControlToGrid(
        "IgnoreMasterLayerBox", "CheckBoxControl", False,
        "Ignore the Master Layer", 5, 2,
        "If enabled, the masterLayer will be ignored in this render.", False)
    ###################################################

    scriptDialog.AddControlToGrid(
        "ProjectLabel", "LabelControl", "Project Path", 6, 0,
        "The Maya project folder (this should be a shared folder on the network).",
        False)
    projBox = scriptDialog.AddSelectionControlToGrid("ProjectBox",
    ...
    scriptDialog.AddControlToGrid("SceneLabel", "LabelControl", "Maya File", 7,
    ...
    scriptDialog.AddControlToGrid(
        "OutputLabel", "LabelControl", "Output Path", 8, 0,
    ...
    ###################################################
    scriptDialog.AddControlToGrid(
        "BuildLabel", "LabelControl", "Build To Force", 9, 0,
        "You can force 32 or 64 bit rendering with this option.", False)
    scriptDialog.AddComboControlToGrid("BuildBox", "ComboControl", "64bit",
                                       ("None", "32bit", "64bit"), 9, 1)
    scriptDialog.AddSelectionControlToGrid(
        "SubmitSceneBox",
        "CheckBoxControl",
        False,
        ...
        colSpan=2)
    mayaBatchBox = scriptDialog.AddSelectionControlToGrid(
        "MayaBatchBox", "CheckBoxControl", True, "Use MayaBatch Plugin", 10, 1,
        "This uses the MayaBatch plugin. It keeps the scene loaded in memory between frames, which reduces the overhead of rendering the job.",
        False)
    mayaBatchBox.ValueModified.connect(MayaBatchChanged)
    scriptDialog.AddSelectionControlToGrid(
        "Ignore211Box", "CheckBoxControl", False, "Ignore Error Code 211", 10,
        2,
        "This allows a MayaCmd task to finish successfully even if Maya returns the non-zero error code 211. Sometimes Maya will return this error code even after successfully saving the rendered images.",
        False)
    scriptDialog.AddSelectionControlToGrid(
    ...
    scriptDialog.AddSelectionControlToGrid(
        "StrictErrorCheckingBox", "CheckBoxControl", True,
        "Strict Error Checking", 11, 2,
        "Enable this option to have Deadline fail Maya jobs when Maya prints out any 'error' or 'warning' messages. If disabled, Deadline will only fail on messages that it knows are fatal."
    )
    ###################################################

    scriptDialog.AddControlToGrid(
        "RendererLabel", "LabelControl", "Renderer", 12, 0,
        "The Maya renderer to use. If you select 'File', the renderer defined in the scene file will be used, but renderer specific options will not be set by Deadline.",
        False)
    rendererBox = scriptDialog.AddComboControlToGrid(
        "RendererBox", "ComboControl", "Arnold",
        ("File", "3delight", "Arnold", "FinalRender", "Gelato", "Maxwell",
    ...
    ###################################################
    scriptDialog.AddControlToGrid(
        "ArnoldVerboseLabel", "LabelControl", "Arnold Verbosity", 13, 0,
        "Set the verbosity level for Arnold renders.", False)
    scriptDialog.AddComboControlToGrid("ArnoldVerboseBox", "ComboControl", "2",
                                       ("0", "1", "2"), 13, 1)
    scriptDialog.AddSelectionControlToGrid(
    ...
    scriptDialog.AddControlToGrid("Separator5",
                                  "SeparatorControl",
                                  "Advanced Maya Options",
                                  0,
                                  0,
                                  colSpan=3)

    scriptDialog.AddControlToGrid(
        "CpusLabel", "LabelControl", "Threads", 1, 0,
        "The number of threads to use for rendering.", False)
    scriptDialog.AddRangeControlToGrid("CpusBox", "RangeControl", 0, 0, 256, 0,
                                       1, 1, 1)
    scriptDialog.AddControlToGrid(
        "FrameNumberOffsetLabel", "LabelControl", "Frame Number Offset", 2, 0,
        "Uses Maya's frame renumbering option to offset the frames that are rendered.",
        False)
    scriptDialog.AddRangeControlToGrid("FrameNumberOffsetBox", "RangeControl",
                                       0, -100000, 100000, 0, 1, 2, 1)
    scriptDialog.AddControlToGrid("VersionLabel", "LabelControl", "Version", 3,
                                  0, "The version of Maya to render with.",
                                  False)
    versionBox = scriptDialog.AddComboControlToGrid(
        "VersionBox", "ComboControl", "2018",
        ("2010", "2011", "2011.5", "2012", "2012.5", "2013", "2013.5", "2014",
         "2014.5", "2015", "2015.5", "2016", "2016.5", "2017", "2017.5",
         "2018", "2018.5", "2018.6", "2020", "2022"), 3, 1)
    versionBox.ValueModified.connect(VersionChanged)

    scriptDialog.AddSelectionControlToGrid(
        "SkipExistingBox", "CheckBoxControl", False, "Skip Existing Frames", 3,
        2,
        "If enabled, Maya will skip rendering existing frames. Only supported for Maya Softwate, Maya Hardware, Maya Vector, and Mentay Ray."
    )

    scriptDialog.AddControlToGrid(
        "StartupScriptLabel", "LabelControl", "Startup Script", 4, 0,
        "Maya will source the specified script file on startup.", False)
    scriptDialog.AddSelectionControlToGrid(
        "StartupScriptBox",
        "FileBrowserControl",
        "",
        "Melscript Files (*.mel);;Python Files (*.py);;All Files (*)",
        4,
        1,
        colSpan=2)

    overrideSizeBox = scriptDialog.AddSelectionControlToGrid(
        "OverrideSizeBox",
        "CheckBoxControl",
        False,
        "Override Resolution",
        5,
        0,
        "Enable this option to override the Width and Height (respectively) of the rendered images.",
        colSpan=1)
    scriptDialog.AddRangeControlToGrid("WidthSizeRange", "RangeControl", 0, 0,
                                       1000000, 0, 1, 5, 1)
    scriptDialog.AddRangeControlToGrid("HeightSizeRange", "RangeControl", 0, 0,
                                       1000000, 0, 1, 5, 2)
    overrideSizeBox.ValueModified.connect(OverrideSizeChanged)
    OverrideSizeChanged()

    scaleBox = scriptDialog.AddSelectionControlToGrid(
        "ScaleBox",
        "CheckBoxControl",
        False,
        "Scale Resolution",
        6,
        0,
        "Enable this option to scale the size of the rendered images to the specified percentage.",
        colSpan=1)
    scriptDialog.AddRangeControlToGrid("ScaleRange", "RangeControl", 100, 0.01,
                                       1000, 2, 1, 6, 1)
    scaleBox.ValueModified.connect(ScaleChanged)
    ScaleChanged()

    scriptDialog.AddSelectionControlToGrid(
        "FileUsesLegacyRenderLayersBox", "CheckBoxControl", False,
        "File Uses Legacy Render Layers", 6, 2,
        "As of Maya 2016.5, Autodesk has added a new render layer system (render setup) that is incompatible with the older version (legacy). This value specifies which render layers system this file uses."
    )
    ...
    scriptDialog.AddControlToGrid(
        ...
        "Specify additional command line arguments you would like to pass to the renderer.",
        False)
    ...
    scriptDialog.AddSelectionControlToGrid(
        ...
        "CheckBoxControl",
        False,
        "Use Only Additional Command Line Arguments",
        2,
        0,
        "Enable this option to only use the command line arguments specified above when rendering.",
        colSpan=2)
    ...
    scriptDialog.AddControlToGrid(
        "ScriptJobLabel1",
        "LabelControl",
        "Script Jobs use the MayaBatch plugin, and do not force a particular render.",
        4,
        0,
        colSpan=2)
    scriptJobBox = scriptDialog.AddSelectionControlToGrid(
        ...
        "CheckBoxControl",
        False,
        "Submit A Maya Script Job (melscript or python)",
        5,
        0,
        "Enable this option to submit a custom melscript or python script job. This script will be applied to the scene file that is specified.",
        colSpan=2)
    scriptJobBox.ValueModified.connect(ScriptJobChanged)

    scriptDialog.AddControlToGrid("ScriptFileLabel", "LabelControl",
                                  "Script File", 6, 0,
                                  "The script file to use.", False)
    scriptDialog.AddSelectionControlToGrid("ScriptFileBox",
                                           "FileBrowserControl", "",
                                           "Maya Script Files (*.mel *.py)", 6,
                                           1)

    scriptDialog.AddControlToGrid(
        "ScriptJobLabel2",
        "LabelControl",
        "All other options on this tab page are ignored, except for the Strict Error Checking option.",
        7,
        0,
        colSpan=2)
    ...
    scriptDialog.AddControlToGrid("MentalRaySeparator",
                                  "SeparatorControl",
                                  "Mental Ray Options",
                                  1,
                                  0,
    ...
    scriptDialog.AddControlToGrid(
        "MentalRayVerboseLabel", "LabelControl", "Mental Ray Verbosity", 2, 0,
        "Set the verbosity level for Mental Ray renders.", False)
    scriptDialog.AddComboControlToGrid(
        "MentalRayVerboseBox", "ComboControl", "Progress Messages",
        ("No Messages", "Fatal Messages Only", "Error Messages",
         "Warning Messages", "Info Messages", "Progress Messages",
         "Detailed Messages (Debug)"), 3, 1)

    autoMemoryLimitBox = scriptDialog.AddSelectionControlToGrid(
        "AutoMemoryLimitBox", "CheckBoxControl", True, "Auto Memory Limit", 3,
        0,
        "If enabled, Mental Ray will automatically detect the optimal memory limit when rendering.",
        False)
    autoMemoryLimitBox.ValueModified.connect(AutoMemoryLimitChanged)

    scriptDialog.AddControlToGrid(
        "MemoryLimitLabel", "LabelControl", "Memory Limit (in MB)", 4, 0,
        "Soft limit (in MB) for the memory used by Mental Ray (specify 0 for unlimited memory).",
        False)
    scriptDialog.AddRangeControlToGrid("MemoryLimitBox", "RangeControl", 0, 0,
                                       100000, 0, 1, 4, 1)

    scriptDialog.AddControlToGrid("VRaySeparator",
                                  "SeparatorControl",
                                  "VRay Options",
                                  5,
                                  0,
    ...
    vrayAutoMemoryBox = scriptDialog.AddSelectionControlToGrid(
        "VRayAutoMemoryBox",
        "CheckBoxControl",
        False,
        "Auto Memory Limit Detection (Requires the MayaBatch Plugin)",
        7,
        0,
        "If enabled, Deadline will automatically detect the dynamic memory limit for VRay prior to rendering.",
        colSpan=2)
    vrayAutoMemoryBox.ValueModified.connect(VRayAutoMemoryChanged)

    scriptDialog.AddControlToGrid(
        "VRayMemoryLimitLabel", "LabelControl", "Memory Buffer (in MB)", 8, 0,
        "Deadline subtracts this value from the system's unused memory to determine the dynamic memory limit for VRay.",
        False)
    scriptDialog.AddRangeControlToGrid("VRayMemoryLimitBox", "RangeControl",
                                       500, 0, 100000, 0, 1, 8, 1)

    scriptDialog.AddControlToGrid("IRaySeparator",
                                  "SeparatorControl",
                                  "IRay Options",
                                  9,
                                  0,
    ...
    irayUseCPUsBox = scriptDialog.AddSelectionControlToGrid(
        "IRayUseCPUsBox", "CheckBoxControl", True, "Render Using CPUs", 10, 0,
        "If enabled, CPUs will be used to render the scene in addition to GPUs."
    )
    irayUseCPUsBox.ValueModified.connect(IRayUseCPUsChanged)
    scriptDialog.AddControlToGrid(
        "IRayCPULoadLabel", "LabelControl", "CPU Load", 10, 1,
        "The maximum load on the CPU for each task.", False)
    scriptDialog.AddRangeControlToGrid("IRayCPULoadBox", "RangeControl", 1.0,
                                       4.0, 8, 1, 0.1, 10, 2)

    scriptDialog.AddControlToGrid("IRayMaxSamplesLabel", "LabelControl",
                                  "Max Samples", 11, 0,
                                  "The Maximum number of samples per frame.",
                                  False)
    scriptDialog.AddRangeControlToGrid("IRayMaxSamplesBox", "RangeControl", 1,
                                       1, 4096, 0, 1, 11, 1)
    ...
    scriptDialog.AddControlToGrid(
        "GPUsPerTaskLabel", "LabelControl", "GPUs Per Task", 13, 0,
    ...
    GPUsPerTaskBox = scriptDialog.AddRangeControlToGrid(
        "GPUsPerTaskBox", "RangeControl", 0, 0, 1024, 0, 1, 13, 1)
    GPUsPerTaskBox.ValueModified.connect(GPUsPerTaskChanged)

    scriptDialog.AddControlToGrid(
        "GPUsSelectDevicesLabel", "LabelControl", "Select GPU Devices", 14, 0,
    ...
    GPUsSelectDevicesBox = scriptDialog.AddControlToGrid(
        "GPUsSelectDevicesBox", "TextControl", "", 14, 1)
...
def VersionChanged(*args):
    ...
    skipExistingEnabled = (float(scriptDialog.GetValue("VersionBox")) >=
                           2014) and (renderer == "MayaHardware"
                                      or renderer == "MayaHardware2"
                                      or renderer == "MayaVector"
                                      or renderer == "MayaSoftware"
                                      or renderer == "MentalRay")
    scriptDialog.SetEnabled("SkipExistingBox", skipExistingEnabled)

    legacyRenderLayersEnabled = float(
        scriptDialog.GetValue("VersionBox")) >= 2016.5
    scriptDialog.SetEnabled("FileUsesLegacyRenderLayersBox",
                            legacyRenderLayersEnabled)
...
def RendererChanged(*args):
    ...
    renderer = scriptDialog.GetValue("RendererBox")
    threadsEnabled = (renderer == "MayaSoftware" or renderer == "MentalRay"
                      or renderer == "Renderman" or renderer == "RendermanRIS"
                      or renderer == "3delight" or renderer == "FinalRender"
                      or renderer == "Maxwell" or renderer == "Vray")
    ...
    scriptDialog.SetEnabled("VRayMemoryLimitLabel", vrayEnabled and memEnabled
                            and batchEnabled)
    scriptDialog.SetEnabled("VRayMemoryLimitBox", vrayEnabled and memEnabled
                            and batchEnabled)
    ...
    gpuEnabled = ((scriptDialog.GetValue("RendererBox") == "Iray")
                  or (scriptDialog.GetValue("RendererBox") == "Redshift")
                  or (scriptDialog.GetValue("RendererBox") == "OctaneRender"))
...
def IRayUseCPUsChanged(*args):
    ...
    scriptDialog.SetEnabled("IRayCPULoadBox",
                            scriptDialog.GetValue("IRayUseCPUsBox"))
...
def SubmitJob(renderlayer, *args):
    ...
    if (not Directory.Exists(projectFolder)):
        scriptDialog.ShowMessageBox(
            "Project folder %s does not exist" % projectFolder, "Error")
        return
    ...
    if (not scriptJob and renderer != "3delight"):
        if (not Directory.Exists(outputFolder)):
            scriptDialog.ShowMessageBox(
                "Output folder %s does not exist" % outputFolder, "Error")
            return
    ...
    if scriptDialog.GetValue("MayaBatchBox") and len(startupScriptFile) > 0:
        if (not File.Exists(startupScriptFile)):
            scriptDialog.ShowMessageBox(
                "Startup Script file %s does not exist" % startupScriptFile,
                "Error")
            return
        elif (PathUtils.IsPathLocal(startupScriptFile)):
            result = scriptDialog.ShowMessageBox(
                "Startup Script file %s is local, are you sure you want to continue?"
                % startupScriptFile, "Warning", ("Yes", "No"))
            if (result == "No"):
                return
    ...
    if (scriptJob):
        if (not File.Exists(scriptFile)):
            scriptDialog.ShowMessageBox(
                "Script file %s does not exist" % scriptFile, "Error")
            return
    ...
    if (not FrameUtils.FrameRangeValid(frames)):
        scriptDialog.ShowMessageBox("Frame range %s is not valid" % frames,
                                    "Error")
        return

    # If Redshift and using 'select GPU device Ids' then check device Id syntax is valid
    if ((renderer == "Redshift" or renderer == "ifmIrayPhotoreal"
         or renderer == "OctaneRender")
            and scriptDialog.GetValue("GPUsPerTaskBox") == 0
            and scriptDialog.GetValue("GPUsSelectDevicesBox") != ""):
...
#######################################################################################
        ...
        if renderlayer:
            if renderlayer == "defaultRenderLayer":
                ...
                # writer.WriteLine("Name=%s" % jobName + " - " + "masterLayer")
                # writer.WriteLine("Name=%s" % jobName + " - " + renderlyr)
            else:
                writer.WriteLine("Name=%s" % jobName + " - " + renderlayer)
        else:
...
#######################################################################################
        ...
        writer.WriteLine("ExtraInfo6=%s" %
                         scriptDialog.GetValue("ProjectCodeBox"))
        writer.WriteLine("ExtraInfo7=%s" %
                         scriptDialog.GetValue("ProjectPhaseBox"))
        writer.WriteLine("ExtraInfo8=%s" %
                         scriptDialog.GetValue("FrameSizeBox"))
        writer.WriteLine("ExtraInfo9=%s" %
                         scriptDialog.GetValue("FileTypeBox"))
...
#############################################################################################################
        if (not scriptJob and renderer != "3delight"):
            filedirectory = "%s/%s/beauty/" % (outputFolder, renderlayer)
            # filedirectory = "%s\\%s\\beauty\\" % (outputFolder, renderlyr)
            # writer.WriteLine("OutputDirectory0=%s" % outputFolder
            writer.WriteLine("OutputDirectory0=%s" % filedirectory)
            fileType = scriptDialog.GetValue("FileTypeBox")
            if fileType == "EXR16":
                ext = "exr"
            else:
                ext = "png"
            filename = scriptDialog.GetValue("OutputBox")
            fileoutput = "%s_%s_beauty_####.%s" % (filename.split('/')[-1], renderlayer, ext)
            # fileoutput = "%s_%s_beauty_####.%s" % (filename.split('/')[-1], renderlyr, ext)
            writer.WriteLine("OutputFilename0=%s" % fileoutput)
            # writer.WriteLine("OutputFilename0=%s" % filename)
#############################################################################################################
        ...
        writer.WriteLine("StrictErrorChecking=%s" %
                         scriptDialog.GetValue("StrictErrorCheckingBox"))
        writer.WriteLine(
            "UseLegacyRenderLayers=%s" %
            int(scriptDialog.GetValue("FileUsesLegacyRenderLayersBox")))

        if scriptJob:
            writer.WriteLine("ScriptJob=True")
            writer.WriteLine("ScriptFilename=%s" %
                             Path.GetFileName(scriptFile))
        else:
            writer.WriteLine("LocalRendering=%s" %
                             scriptDialog.GetValue("LocalRenderingBox"))
            writer.WriteLine("MaxProcessors=%s" %
                             scriptDialog.GetValue("CpusBox"))
            writer.WriteLine("FrameNumberOffset=%s" %
                             scriptDialog.GetValue("FrameNumberOffsetBox"))
            ...
            if (renderer == "Arnold"):
                writer.WriteLine("ArnoldVerbose=%s" %
                                 scriptDialog.GetValue("ArnoldVerboseBox"))
            elif (renderer == "MentalRay"):
                writer.WriteLine("MentalRayVerbose=%s" %
                                 scriptDialog.GetValue("MentalRayVerboseBox"))
                writer.WriteLine("AutoMemoryLimit=%s" %
                                 scriptDialog.GetValue("AutoMemoryLimitBox"))
                writer.WriteLine("MemoryLimit=%s" %
                                 scriptDialog.GetValue("MemoryLimitBox"))
            elif (renderer == "Vray"
                  and scriptDialog.GetValue("MayaBatchBox")):
                writer.WriteLine("VRayAutoMemoryEnabled=%s" %
                                 scriptDialog.GetValue("VRayAutoMemoryBox"))
                writer.WriteLine("VRayAutoMemoryBuffer=%s" %
                                 scriptDialog.GetValue("VRayMemoryLimitBox"))
            elif (renderer == "ifmIrayPhotoreal"):
                writer.WriteLine("IRayUseCPUs=%s" %
                                 scriptDialog.GetValue("IRayUseCPUsBox"))
                writer.WriteLine("IRayCPULoad=%s" %
                                 scriptDialog.GetValue("IRayCPULoadBox"))
                writer.WriteLine("IRayMaxSamples=%s" %
                                 scriptDialog.GetValue("IRayMaxSamplesBox"))
            elif (renderer == "Redshift" or renderer == "ifmIrayPhotoreal"
                  or renderer == "OctaneRender"):
            ...
            writer.WriteLine("CommandLineOptions=%s" %
                             scriptDialog.GetValue("CommandLineBox").strip())
            writer.WriteLine("UseOnlyCommandLineOptions=%d" %
                             scriptDialog.GetValue("UseCommandLineBox"))
            writer.WriteLine("IgnoreError211=%s" %
                             scriptDialog.GetValue("Ignore211Box"))

            if (float(scriptDialog.GetValue("VersionBox")) >= 2014) and (
                    renderer == "MayaHardware" or renderer == "MayaHardware2"
                    or renderer == "MayaVector" or renderer == "MayaSoftware"
                    or renderer == "MentalRay"):
                writer.WriteLine("SkipExistingFrames=%s" %
                                 scriptDialog.GetValue("SkipExistingBox"))

            if scriptDialog.GetValue("OverrideSizeBox"):
                writer.WriteLine("ImageWidth=%s" %
                                 scriptDialog.GetValue("WidthSizeRange"))
                writer.WriteLine("ImageHeight=%s" %
                                 scriptDialog.GetValue("HeightSizeRange"))

            if scriptDialog.GetValue("ScaleBox"):
                writer.WriteLine("ImageScale=%s" %
                                 scriptDialog.GetValue("ScaleRange"))

        ##########################################################

            if scriptDialog.GetValue("CameraBox"):
                writer.WriteLine("Camera=%s" %
                                 scriptDialog.GetValue("CameraBox"))
