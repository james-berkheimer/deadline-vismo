...
def __main__( *args ):
    ...
    scriptDialog.SetIcon( Path.Combine( RepositoryUtils.GetRootDirectory(), "plugins/MayaMDDCmd/MayaMDDCmd.ico" ) )
...
###################################################    
    scriptDialog.AddControlToGrid( "ProjectPhaseLabel", "LabelControl", "Project Phase", 3, 0, "The Project you belong to. This is optional and can be left blank.", False )
    scriptDialog.AddControlToGrid( "ProjectPhaseBox", "TextControl", "", 3, 1 )

    scriptDialog.AddControlToGrid( "FileTypeLabel", "LabelControl", "File Type", 4, 0, "The Project you belong to. This is optional and can be left blank.", False )
    scriptDialog.AddControlToGrid( "FileTypeBox", "TextControl", "", 4, 1 )
    
    scriptDialog.AddControlToGrid( "FramesLabel", "LabelControl", "Frames", 5, 0, "The frame range of the export", False )
    scriptDialog.AddControlToGrid( "FrameBox", "TextControl", "", 5, 1 )   
###################################################
    ...
    # scriptDialog.AddControlToGrid( "TestLabel", "LabelControl", "Testing", 7, 0, "I need to put things in here." )
    # scriptDialog.AddControlToGrid( "TestBox", "TextControl", "", 7, 1, colSpan=3 )  
    scriptDialog.EndGrid()       
...
###################################################################################################################################  
    ...
    # settings = ("PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","MachineLimitBox","IsBlacklistBox","LimitGroupBox","FramesBox","ChunkSizeBox")
    # scriptDialog.LoadSettings( GetSettingsFilename(), settings )
    # scriptDialog.EnabledStickySaving( settings, GetSettingsFilename() )
    
    scriptDialog.SetValue( "PoolBox", "push_priority" )
    scriptDialog.SetValue( "GroupBox", "maya" )
    
    appSubmission = False
    
    if len( args ) > 0:
        argsList = [' -ms ', ' -ep ', ' -fs ', ' -fe ', ' -ml ', ' -sl ', ' -oe ']
        scriptDialog.SetValue( "ExecBox", args[0] )
        # cmd_arg = args[1] + ' -ms ' + args[2] + ' -ep ' + args[3] + ' -fs ' + args[4] + ' -fe ' + args[5] + ' -ml ' + args[6] + ' -sl ' + args[7] + ' -sv ' + args[8] + ' -sfx ' + args[9]
        
        argsFlagList = [' -ms ', ' -ep ', ' -fs ', ' -fe ', ' -ml ', ' -sl ', ' -oe ']
        argsDict = {}   
        for i in range(len(args[2:])):
            if args[i+2]:
                argsDict[argsFlagList[i]] = args[i+2]
                
        cmd_arg = args[1]
        
        for key, value in argsDict.iteritems():
            cmd_arg = cmd_arg + key + value        
        
        scriptDialog.SetValue( "ArgsBox", cmd_arg )
        
        fileInfo = args[2].split('/')
        sceneName = fileInfo[-1].split('.')[0]
        scriptDialog.SetValue( "NameBox", sceneName )
        scriptDialog.SetValue( "ProjectPhaseBox", "ANIM" )
        scriptDialog.SetValue( "FileTypeBox", "MDD" )
        
        frames = ""
        if args[4]:
            frames = args[4] + '-' + args[5]
            scriptDialog.SetValue( "FrameBox", frames )
            
        ####################
        # scriptDialog.SetValue( "TestBox", "Hello")
        ####################
...
def GetSettingsFilename():
    ...
    return Path.Combine( GetDeadlineSettingsPath(), "MayaMDDCmdSettings.ini" )
...
def SubmitButtonPressed(*args):
    ...
    jobInfoFilename = Path.Combine( GetDeadlineTempPath(), "mayacmd_job_info.job" )
    writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
    # writer.WriteLine( "Plugin=CommandLine" )
    writer.WriteLine( "Plugin=MayaMDDCmd" )
...
#######################################################################################
    ...
    writer.WriteLine( "ExtraInfo8=%s" % "N/A" )
...
#######################################################################################
    ...
    # frames = scriptDialog.GetValue( "FrameBox" )
    ...
    pluginInfoFilename = Path.Combine( GetDeadlineTempPath(), "mayacmd_plugin_info.job" )
